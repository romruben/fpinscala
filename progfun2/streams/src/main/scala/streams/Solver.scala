...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    for {
      (block, move) <- neighbors if !explored.contains(block)
    } yield (block, move)
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (b, ml) #:: tail =>
      val next = newNeighborsOnly(neighborsWithHistory(b, ml), explored + b)
      next ++ from(tail ++ next, explored + b)
  }
