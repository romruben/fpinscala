...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("[parallel] countChange should return 0 for money < 0") {

    def check(money: Int, coins: List[Int]) = {
      val threshold = totalCoinsThreshold(coins.length)
  ...
  test("[parallel] countChange should return 1 when money == 0") {
    def check(coins: List[Int]) = {
      val threshold = totalCoinsThreshold(coins.length)
