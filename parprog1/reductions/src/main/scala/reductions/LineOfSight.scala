...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while (it < until) {
      output(it) = (input(it) / it) max output(it - 1)
      it += 1
    }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    while(it < until){
      maxAngle = maxAngle max (input(it) / it)
      it += 1
    }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    case node : Node => parallel(
      downsweep(input, output, startingAngle, node.left),
      downsweep(input, output, node.left.maxPrevious max startingAngle, node.right)
    )
  }
