...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.isEmpty) means.map(means => means -> GenSeq.empty[Point]).toMap
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    !oldMeans.zipWithIndex.exists{
      case (point: Point, idx: Int) => point.squareDistance(newMeans(idx)) > eta
    }
  }
