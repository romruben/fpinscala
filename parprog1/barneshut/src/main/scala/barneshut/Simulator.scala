...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    ...
    mergedBoundary.maxX = a.maxX max b.maxX
    mergedBoundary.maxY = a.maxY max b.maxY
    mergedBoundary.minX = a.minX min b.minX
    mergedBoundary.minY = a.minY min b.minY
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    ...
    parBodies.aggregate(new SectorMatrix(boundaries, SECTOR_PRECISION))(
      (sector, body) => sector += body,
      (sector, otherSector) => sector combine otherSector)
  }
